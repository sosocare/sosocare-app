...
import { ApplicationState, UserState, OnContactSupport } from '../../../redux';
...
const _SupportScreen: React.FC<SupportScreenProps> = (props) => {
    const { userReducer, OnContactSupport } = props;
    ...
    const handleSubmit = async ({ subject, message }) => {
        setLoading(true);
        try {
            let res = await OnContactSupport(subject, message);
            if (res.error) {
                setError(res.error);
                setLoading(false);
                setHasError(true);
...
const SupportScreen = connect(mapToStateProps, { OnContactSupport })(_SupportScreen);
