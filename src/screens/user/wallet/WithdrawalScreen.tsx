...
const _WithdrawalScreen: React.FC<WithdrawalScreenProps> = (props) => {
    const { userReducer, walletReducer, OnWithdrawFunds, OnResolveBank, OnSavePaymentMethod, OnLoadWallet, OnVerifyTransaction, OnLoadBanks } = props;
    ...
    const addWithdrawalMethod = async () => {
        ...
        if (accountName && accountNumber && bank && bank_code) {
            try {
                setLoading(true);
                let res = await OnSavePaymentMethod(accountNumber, bank_code, bank, accountName, 'user', bvn);
                if (res.error) {
                    setLoading(false);
                    setError(res.error);
                    setHasError(true);
    ...
    const WithdrawFunds = async (amount) => {
        try {
            ...
            let res = await OnWithdrawFunds(amount, 'user');
            if (res.error) {
                setLoading(false);
                setError(res.error);
                setHasError(true);
...
const WithdrawalScreen = connect(mapToStateProps, { OnLoadWallet, OnWithdrawFunds, OnSavePaymentMethod, OnResolveBank, OnVerifyTransaction, OnLoadBanks })(_WithdrawalScreen);
