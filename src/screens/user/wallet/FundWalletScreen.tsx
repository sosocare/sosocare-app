...
import { ApplicationState, UserState, WalletState, WalletModel, SosocareBankModel, OnLoadWallet, OnVerifyTransaction, OnGetPayStackKey } from '../../../redux';
...
const _FundWalletScreen: React.FC<FundWalletScreenProps> = (props) => {
    const { userReducer, walletReducer, OnLoadWallet, OnVerifyTransaction, OnGetPayStackKey } = props;
    ...
    const handlePayDone = async (reference) => {
        try {
            ...
            let res = await OnVerifyTransaction(reference, 'user');
            if (res.pending) {
                setVerifying(false);
                setLoading(false);
                setPayingWithPayStack(false);
    ...
    const stepList = [
        ...
        {
            content: <Transfer sosocareBank={sosocareBank} wallet={userWallet} handleSubmit={() => { }} loading={loading} />,
        },
        {
...
const Main = ({ handleSubmit, loading }) => {
    ...
    return (
        <View style={{ ...styles.stepContainer }}>
            ...
            />
            <ListItemAlt
                title='Recyclable Wastes'
                backgroundColor='white'
                color={COLORS.dark}
                tint={COLORS.fade}
                loading={false}
                onTap={() => handleSubmit(3)}
                width={'100%'}
                leftIcon={'recycle-variant'}
            />
        </View>
    );
};
